...
export const mockUsers: User[] = Array.from({ length: 50 }, (_, i) => {
  ...
  const roles: UserRole[] = ['super_admin', 'admin', 'agent', 'customer'];
...
export const mockTickets: Ticket[] = Array.from({ length: 200 }, (_, i) => {
  ...
  const assignee = status !== 'New' ? mockUsers.filter(u => u.role === 'agent')[Math.floor(Math.random() * mockUsers.filter(u => u.role === 'agent').length)] : undefined;
...
export const mockChartData = {
  ...
  ticketsByAgent: mockUsers
    .filter(u => u.role === 'agent')
    .map(agent => ({
      name: `${agent.first_name} ${agent.last_name}`,
      count: mockTickets.filter(t => t.assignee_id === agent.id).length,
    }))
